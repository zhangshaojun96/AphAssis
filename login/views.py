...
def after_login(request):
    ...
    return render(request, 'index.html', {'username': Login_user, 'classid': classid})
...
def nregister(request):
    ...
    if request.method == 'POST':
        ...
        # 类型是2表示是登录
            ...
            m = register.objects.get(res_username=username, res_password=password)
            if m:
                # user_id = m.id

                Login_user = username
                request.session['username'] = Login_user
                request.session['classid'] = m.res_id
